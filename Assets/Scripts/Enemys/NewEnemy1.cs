...
public class NewEnemy1 : MonoBehaviour
{
    ...
    [SerializeField] private float _timeBetwennWps = 1.5f;

    private bool _canShootPlayer = false;
    [SerializeField] private Transform _wpForInvisibility;
    ...
    private void Update()
    {
        ...
        if (gameObject != null)
        {
            if (_agent.isOnNavMesh)
            {
                ...
                else if (_distanceFromPlayer <= _minDistanceToShootPlayer)
                {
                    _canShootPlayer = true;
                }
                else if (_distanceFromPlayer > _minDistanceToFollowPlayer)
                {
                ...
                if (_invisibility != null && _invisibility.isInvisible)
                {
                    ...
                    _agent.SetDestination(_wpForInvisibility.position);
                }
    ...
    private IEnumerator Patroll()
    {
        if (_agent.isOnNavMesh)
        {
            Vector2 dirToPlayer = (_playerTransform.position - _sight.position).normalized;
            Quaternion bulletRotation = Quaternion.LookRotation(Vector3.forward, dirToPlayer);
            ...
            {
                _wpsIndex = Random.Range(0, _wayPoints.Length);
                yield return new WaitForSeconds(_timeBetwennWps);
                //_wpsIndex++;
